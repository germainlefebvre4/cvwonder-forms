...
The system SHALL apply the same background-color highlight as hovering when a form field is clicked, SHALL keep that highlight in place after the pointer leaves the field (until another field is hovered or clicked), and SHALL scroll the YAML preview pane so the highlighted range is visible.
